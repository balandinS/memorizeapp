...
export const signinScheme = () => (
    yup.object().shape({
        ...
        .string()
        .email('wrong email')
        .matches(/^((?!\.)[\w\-_.]*[^.])(@\w+)(\.\w+(\.\w+)?[^.\W])$/, 'wrong email')
        ...
        .matches(/^(?=.*[A-Za-z])(?=.*\d)(?!.*\W).{6,8}$/, 'requird number and characters')
        .required("Required field"),
...
export const signupScheme = () => (
    yup.object().shape({
        ...
        .matches(/(^[A-Za-z]{2,16})([ ]{0,1})([A-Za-z]{2,16})?([ ]{0,1})?([A-Za-z]{2,16})?([ ]{0,1})?([A-Za-z]{2,16})$/, 'plaese enter name without number and symbols')
        .required("Required field"),
        email: yup
        .string()
        .email('wrong email')
